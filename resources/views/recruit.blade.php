...
@section('content')
    ...
    <div class="relative py-4 md:py-24 bg-cover bg-top bg-no-repeat"
        ...
        <div class="md:pt-0 pt-8 mt-0 flex flex-row items-center gap-8 z-20">
            <img src="{{ asset($titleRecruit->file_path . $titleRecruit->file_name) }}" alt="タイトル" class="md:h-18 h-8"
                style="object-fit: contain;">
        </div>
        <div class="container md:w-[70%] mx-auto p-4 py-8">
            {{-- Add the content for your recruitment page here --}}
            <div class="md:mt-4 py-6 px-4 " style="background-color: #faf4dd">
                <h2 class="text-2xl md:text-4xl text-center text-orange-400 mb-6">募集情報</h2>

                <div x-data="{ activeTab: 1 }" class="md:max-w-2xl w-full mx-auto mx-2 text-white">
                    {{-- タブタイトル部分 --}}
                        ...
                        @foreach ($textRecruitment as $recruitment)
                            ...
                            <li @class([
                                ...
                                <a href="#"
                                    ...
                                    <p class="text-[14px] md:text-xl font-bold">
                                        {{ $recruitment->industry['value'] }}<br>
                                        <span
                                            class="px-2 md:px-4 text-[8px] md:text-sm font-normal bg-amber-50 text-orange-500">{{ $recruitment->employment_type['value'] }}</span>
                                    </p>
                                </a>
                            </li>
                        @endforeach
        ...
        <div class="hidden md:block">
            <div class="mt-24">
                <span class="pr-8 pt-4 pb-2 text-4xl bg-custom-orange-active underline" style="color: #fcebbe;">
                    　　　採用に関するお問い合わせ</span>
            </div>
            <div class="container w-[80%] mx-auto ">
                <div class="relative mt-24 flex flex-row items-start justify-center" style="color: #f39e10;">
                    <div class="py-4 px-4 rounded-2xl flex-shrink-0 relative z-10"
                        ...
                        <div class=" flex items-center gap-4 text-xl rounded-2xl py-4 px-6">
                            <img src="{{ asset($mailIcon->file_path . $mailIcon->file_name) }}" alt="メール"
                                class="h-14 flex-shrink-0" style="object-fit: contain;">
                        ...
                        <p class="text-2xl font-bold text-center py-2 px-8 tracking-[0.2em]">
                            直接応募頂いた方を優先いたします</p>
                        ...
                        <div class="mx-8 border-2 border-orange-400 p-1">
                            <div class=" bg-custom-orange-active">
                                <p class="text-xl text-white py-4 px-8">
                                    直接応募された方で半年間勤務継続された方には<br>
                                    お祝い金を支給致します。
                                </p>
                            </div>
                        </div>
                        <p class="text-base">※正社員薬剤師100,000円 / パート薬剤師50,000円/パート医療事務20,000円</p>
                    </div>
                ...
                <div class="mt-24 px-12 py-12 rounded-4xl bg-white xl:max-w-4xl max-w-3xl mx-auto shadow-2xl"
                    style="box-shadow: 8px 8px 0px 0px rgba(243, 158, 16,0.3);">
                    <div class="-mt-12 flex flex-row items-start gap-8 z-20">
                        <div class="relative">
                            <img src="{{ asset($titleBg->file_path . $titleBg->file_name) }}" alt="背景" class="h-120"
                                style="object-fit: contain;">
                            <div class="absolute inset-0 flex items-center justify-center">
                                <p class="text-3xl  text-white vertical-text"
                        ...
                        <div class="flex flex-col gap-4 justify-right flex-1">
                            @foreach($qa as $qaItem)
                                <img src="{{ asset($qaItem->file_path . $qaItem->file_name) }}" alt="QA"
                                    class="mt-12 mx-12 w-[70%]" style="object-fit: contain;">
                            @endforeach
                        </div>
                    </div>
                </div>
        ...
        <div class="md:hidden ">
            ...
            <div class="mx-2 my-12">
                <img src="{{ asset($qaContent->file_path . $qaContent->file_name) }}" alt=""
                    class="w-full md:w-[80%] mx-auto " style="object-fit: contain;">
            </div>
        </div>
